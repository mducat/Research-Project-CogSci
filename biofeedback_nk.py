...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.analysis_label.setFixedSize(300, 950)
        ...
        self.plot_layout.addWidget(self.interval_graph)
        self.plot_layout.addWidget(self.fft_graph)
    ...
    def update_draw(self):
        ...
        if len(recorded_data) < 1000:
            self.plot_line = self.plot_graph.plot(recorded_data[-2000:])
            return

        self.data = recorded_data
        df, info = nk.ppg_process(recorded_data, sampling_rate=100)

        # Analyze
        analyze_df = nk.ppg_analyze(df, sampling_rate=100)
        label = ""
        for col in analyze_df.columns:
            if np.isnan(analyze_df[col][0]):
                continue
            label += f"{col}={analyze_df[col][0]}\n"
        self.analysis_label.setText(label)

        self.plot_line = self.plot_graph.plot(df["PPG_Clean"][-2000:].to_numpy())
        ...
        low_band = 0.04  # Hz
        high_band = 0.26  # Hz
        lf_mask = (xf >= low_band) & (xf <= high_band)
        window = 0.03  # Hz
        ...
        if np.sum(mag[lf_mask]) > 0:
            ...
            below_mask = (xf >= low_band) & (xf < lf_peak_freq - window / 2)
            above_mask = (xf <= high_band) & (xf > lf_peak_freq + window / 2)

            peak_freq_power = np.sum(mag[win_mask])
            freq_power_below = np.sum(mag[below_mask])
            freq_power_above = np.sum(mag[above_mask])

            if freq_power_below and freq_power_below and freq_power_above:

                component1 = peak_freq_power / freq_power_below
                component2 = peak_freq_power / freq_power_above

                self.coherence_score = np.log(component1 * component2 + 1)

            # CR = (Peak Power/Total power below the peak frequency)*(Peak Power/Total power above peak frequency)

            self.hr_label.setText(f"HR: {hr:.1f} bpm\n coherency score: {self.coherence_score:.2f}\npeak freq: {lf_peak_freq:.2f} Hz")
